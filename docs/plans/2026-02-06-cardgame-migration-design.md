...
**Date:** 2026-02-06
**Goal:** Migrate the 20250126-card_game02 frontend/backend into the new cardgame project with a new API/WS prefix, and keep future iteration in the new project.

## Scope
- Frontend: copy the entire `frontend/project/20250126-card_game02` into `frontend/project/cardgame`.
- Backend: create a new `backend/projects/cardgame.js` that mirrors `20250126-card_game02.js` behavior, but with a new API prefix `/api/cardgame` and updated health metadata.
...
- All REST/WS traffic is re-pointed to the new prefix `/api/cardgame`.
...
1. Copy `20250126-card_game02` to `cardgame`.
2. Update frontend API/WS prefix references to `/api/cardgame`.
3. Clone backend module to `cardgame.js` and update prefix + health metadata.
...
## Success Criteria
- New frontend project connects to backend using `/api/cardgame`.
